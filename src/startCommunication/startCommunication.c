...

{
    ...
    for (int process=0,indx=0; process < worldSize; indx += recvCount[process++] ) {
        ...
        if (sendCount[process] > 0) {                   // need to send to process j
            ...
            MPI_Send(compressedVec[process],sendCount[process],MPI_MY_REAL,process, 231,MPI_COMM_WORLD );
        } // end if //
    } // end for //    
